...
export const selectedLanguage = () => {
  ...
  const setLanguage = (lang: Lang) => localStorage.setItem('lang', lang.toLowerCase())

  const textsToChange = document.querySelectorAll('[data-section]') as unknown as HTMLElement[]

  const changeLanguage = async (lang = language) => {
    const texts = await import(`../lang/${lang}.json`).then((module) => module.default)
